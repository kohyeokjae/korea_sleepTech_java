...
// name: '홍길동', name: '고길동' (불가! - name: '고길동'만 존재)
...
public class E_Map {
	...
	public static void main(String[] args) {
		...
		// {홍길동=25, 홍길덩=27, 고길동=55}
		// : {키=값, 키=값 .. } - 키와 값의 구분을 = 로 표시
		// >> 삽입 순서 보장 X
		
		// 2. get(키): 지정된 키의 값을 반환
		System.out.println(students.get("고길동"));
		
		// 3. remove(키): 지정된 키의 값을 삭제 후 반환
		students.remove("홍길동");
		System.out.println(students);	// {홍길덩=27, 고길동=55}
		
		// 4. containsKey(키): 해당 키가 Map에 있는지 확인하여 boolean 값을 반환
		System.out.println(students.containsKey("홍길동"));	// false
		System.out.println(students.containsKey("고길동"));	// true
		
		// 5. size()
		System.out.println(students.size());	// 2
		
		// 6. keySet(): Map의 키를 Set으로 반환
		System.out.println(students.keySet()); 	// [홍길덩, 고길동]
		
		// 7. values(): Map의 값을 컬렉션타입으로 반환
		Collection<Integer> ages = students.values();
		System.out.println(ages); 	// [27, 55]
		
		// cf) Collection 타입은 하위 인터페이스로 변환 가능
		List<Integer> listAges = new ArrayList<>(ages);
		System.out.println(listAges); 	// [27, 55]
		
		// 8. entrySet(): Map의 키와 값을 Set으로 반환
		System.out.println(students.entrySet());	// [홍길덩=27, 고길동=55]
	}

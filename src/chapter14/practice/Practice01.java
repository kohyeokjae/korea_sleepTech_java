...
public class Practice01 {
	...
	public static void main(String[] args) {
		List<StudentClass> studentClasses = Arrays.asList(
					new StudentClass("홍길동", 20, 4.1),
					new StudentClass("홍길덩", 20, 4.0),
					new StudentClass("홍길둥", 20, 4.2),
					new StudentClass("고길당", 20, 3.0),
					new StudentClass("고길동", 20, 3.3)
				);
		
		// 1) 학점이 4.0 이상인 학생 필터링
		List<StudentClass> filterStudents = studentClasses.stream()
		...
		// StudentClass(name=홍길동, age=20, grade=4.1)
		// StudentClass(name=홍길덩, age=20, grade=4.0)
		// StudentClass(name=홍길둥, age=20, grade=4.2)
		...
		System.out.println(studentNames);	// [홍길동, 홍길덩, 홍길둥, 홍길당, 고길동]
		...
		// 4) 학생들 중 이름에 '홍'가 포함된 학생 필터링
		List<StudentClass> studentWithKo = studentClasses.stream()
				.filter(studentClass -> studentClass.getName().contains("고"))
				.collect(Collectors.toList());
		System.out.println("이름에 '고'이 들어간 학생");
		studentWithKo.forEach(System.out::println);
	}
}

...
public class JavaLang {
	...
	public static void main(String[] args) {
		...
		MyClass myClass1 = new MyClass(1, "홍길동");
		System.out.println(myClass1);
		// MyClass(ID: 1 / NAME: 홍길동)
		// : 객체를 출력하는 경우 자동으로 toString() 호출 - 생략 가능
		System.out.println(myClass1.toString());
		
		System.out.println(myClass1.displayInfo());	// ID: 1 / NAME: 홍길동
		
		MyClass myClass2 = new MyClass(2, "홍길둥");
		...
		System.out.println(Math.max(10, 30)); 	// 최대값 반환: .max(데이터1, 데이터2)
		System.out.println(Math.min(10, 30)); 	// 최대값 반환: .min(데이터1, 데이터2)
		...
		// : 시작 인데스(포함)부터 끝 인덱스(미포함)까지의 부분 문자열 반환

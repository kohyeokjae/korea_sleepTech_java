...
/*
 	...
 	try {
 		예외가 발생할 수 있는 코드
 	} catch('발생할 예외의 타입' '저장할 예외를 저장할 변수') {
 		해당 예외 발생 시 실행할 코드
 	} catch('발생할 예외의 타입' '저장할 예외를 저장할 변수') {
 		해당 예외 발생 시 실행할 코드
 	}
...
public class Exception02 {
	...
	public static void main(String[] args) {
		...
		// at chapter09.Exception02.main(Exception02.java:33)
		
		try {
			// 예외 발생 가능성 코드 작성
			int division = 10 / 0;
		} catch (ArithmeticException e) {	// 예외 변수명: e, exception 등이 주로 사용
			// try 블록 내에서 밸생하는 예외를 매개변수로 자동 전달 받음
	    ...
	    try {
			...
			System.out.println("53번째 줄 주석 시 실행");
			...
			System.out.println("배열 인데스 초과 에러: " + e.getMessage());

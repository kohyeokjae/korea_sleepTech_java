...
/*
 	...
 	: 자바에서는 "함수"의 개념 X
 	: 모든 함수 체계는 "클래스 내부"에 존재 >> "메서드"로 불린다.
 	 ...
 	 >> 필요한 개수만큼 선언, 데이터 타입 생량 X
 	 
 	 +) 구현부
 	 : 반환타입이 void가 아닐경우 반드시! 'return 반환타입데이터;' 포함해야 함!
 	 >> 반환값이 명시된 반환타입과 일치하거나 자동 형 변환이 가능한 데이터
...
class Example {
	...
	// 반환값 X, 매개변수 X
	void print99dan () {	// 메서드명은 변수명의 명명규치과 동일
		for (int i = 0; i < 8; i++) {
			for (int j = 0; j < 8; j++) {
...
public class B_Method {
	...
	public static void main(String[] args) {
		...
		// 함수의 파라미터에 데이터 전달
		// : 함수 호출 시 () 영역 내에서 parameter(매개변수)에 전달되는 값을 argument라고 부른다.
		...
		example.add('1', '2');	// 자동 형 변환가능: char(2) < int(4)

...
public class PatientServiceImpl implements PatientService {
	...
	public void registerPatient(PatientRequestDto dto) {
		Patient patient = new Patient(generateId(), dto.getName(), dto.getAge(), dto.getGenter());
	...
	public PatientResponseDto getPatientById(Long id) {
		...
		try {
			Patient patient = repository.findById(id)
					.orElseThrow(() -> 
					new IllegalArgumentException("해당 ID를 가진 환자를 조회할 수 없습니다. ID: " + id));
	...
	public void updatePatient(Long id, PatientRequestDto dto) {
		try {
			...
			patient.setGender(dto.getGenter());
